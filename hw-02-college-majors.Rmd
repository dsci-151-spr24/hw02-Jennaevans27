...
## Which major has the lowest unemployment rate?
*** Mathematics And Computer Science has the lowest unemployment rate.***
...
***We use the median instead of the mean to describe typical income because it is not affected by outliers or skewed distributions, making it a it a more accurate representation of what most people earn.*** 
...
4.  Based on the shape of the histogram you created in the previous exercise, determine which of these summary statistics is useful for describing the distribution.
   ...
   ***The histograme is skewed to the left,therefor the median is bet at describing the distribution, becuase it is not affected by the extream lows and highes (outliers) of the data rather the middle points*** 
...
ggplot(data = college_recent_grads, 
       mapping = aes(x = women, y = median, colour = major_category %in% stem_categories)) +
  geom_point(alpha = 0.5) +
  labs(
    x = "Proportion of women",
    y = "Median income",
    title = "Distribution of median earnings by major_category",
    colour = "major_category %in% stem_categories"
  )
```

***The assiocation is positive for regular majors and women but there is a less proportion of women who major in Stem but have a higher median income.*** 

## Further exploration

10. Ask a question of interest to you, and answer it using summary statistic(s) and/or visualization(s).

***What is the relationship of median income vs. proportion of men in stem majors vs non stem majors?***  

**Both catogories have a postive relationship, with men in stem majors is stronger and have a higher median income than men who don't major in stem.**

```{r scatterplot_02}
ggplot(data = college_recent_grads, 
       mapping = aes(x = men, y = median, colour = major_category %in% stem_categories)) +
  geom_point(alpha = 0.5) +
  labs(
    x = "Proportion of men",
    y = "Median income",
